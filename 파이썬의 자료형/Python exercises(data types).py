...
print(13%2)
## Result : 1 // % 연산자를 이용해 13을 나눈 나머지가 1이기에 13은 홀수 이다.
...
print(pin[7])
## Result : 1 / 성별을 나타내는 숫자는 하이픈을 포함하여 8번째 숫자이므로 8번째 자리를 인덱싱한다.
...
print(a)
## Result : [5, 4, 3, 2, 1] / 리스트의 내장 함수인 sort를 사용하여 리스트 값들을 먼저 정렬한 후 reverse 함수를 사용하여 순서를 뒤집는다.

# Q7 ['Life', 'is', 'too', 'short'] 리스트를 Life is too short 문자열로 만들어 출력해 보자. ※ 문자열의 join 함수를 사용하면 리스트를 문자열로 쉽게 만들 수 있다.
...
print(t1+t2)
## Result : (1, 2, 3, 4) / a 튜플에 (4, )라는 튜플을 더하면 된다.
...
print(result)
## Result : {'A': 90, 'C': 70}
##          80 / ‘B’ 키 값에 해당되는 값이 리턴되고 딕셔너리 a에서는 그 값이 제거되는 것을 확인할 수 있다.
...
print(b)
## Result : {1, 2, 3, 4, 5} / 리스트 자료형이 집합 자료형으로 변환되면서 중복된 값들은 사라지게 된다.
