...
# 부모클래스가 만든 클래스를 자식 클래스를 적용 할 수 있다.
...
class SafeFourCal(FourCal):
    def div(self):
        if self.second == 0:
        # 두번째 입력값이 0인경우 부모 클래스의 함수를 무시하고 실행됨.
            return 0
        else:
...
print(a.div())
## Result : 0
# 원래는 에러가 나야 하지만 자식클래스의 함수가 부모클래스의 함수를 덮어
...
Family.lastname = "박"
# 클래스 변수를 "박"으로 바꾸었기 때문에 모든 lastname값이 박으로 출력 되었다.
